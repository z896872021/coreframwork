...
public class GlideImageLoaderStrategy implements BaseImageLoaderStrategy<ImageConfigImpl>, GlideAppliesOptions {
    ...
    public void loadImage(@Nullable Context ctx, @Nullable ImageConfigImpl config) {
        ...
        requests = GlideCore.with(ctx);//如果context是activity则自动使用Activity的生命周期
    ...
    public void clear(@Nullable final Context ctx, @Nullable ImageConfigImpl config) {
        ...
        if (config.getImageView() != null) {
            GlideCore.get(ctx).getRequestManagerRetriever().get(ctx).clear(config.getImageView());
        }

        if (config.getImageViews() != null && config.getImageViews().length > 0) {//取消在执行的任务并且释放资源
            for (ImageView imageView : config.getImageViews()) {
                GlideCore.get(ctx).getRequestManagerRetriever().get(ctx).clear(imageView);
            }
        }
